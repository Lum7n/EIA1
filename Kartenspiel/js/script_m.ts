...
let activeCards: Card[] = []; // Array für Spielstapel
...
function shuffleAndPlay(): void {
    ...
    // Abwechselnd Karten an Spieler und Computer (Gegner) verteilen, bis jeder 4 Karten hat
    let i: number  = 4;
    while (i) {
    ...
    if (currentPlayer) {
        ...
        if (cardToPlay.cardColor == currentCard.cardColor || cardToPlay.cardValency == currentCard.cardValency) {
            ...
            if (playerCards.length == 0) {
                setTimeout(function (): void {document.getElementById("currentMove").innerHTML = "Du hast gewonnen!"; clearAll(); } , 500);
            }
            else {
...
function opponent(): void {
    ...
    for (let i: number  = 0; i < computerCards.length; i++) {
        ...
        if (computerCards[i].cardColor == currentCard.cardColor || computerCards[i].cardValency == currentCard.cardValency) {
            ...
            /* fehler, idee reihenfolge ändern, erst ziehen, dann kontrolle ob noch karten vorhanden
            ...
            */ 
            ...
            setTimeout(function (): void {updateHtml(activeCards); updateHtml(computerCards); }, 1500);

            // Überprüfe ob Gegner keine Karten mehr hat, also gewonnen hat
            if (computerCards.length == 0) {
                setTimeout(function (): void {document.getElementById("currentMove").innerHTML = "Du hast verloren!"; clearAll(); }, 2000);
            }
            else {
                // Dann ist der Spieler wieder an der Reihe
                setTimeout(function (): void {document.getElementById("currentMove").innerHTML = "Du bist dran!"; currentPlayer = true; }, 1500);
            }
    ...
    if (couldLay == false) {
        ...
        setTimeout(function(): void {updateHtml(cardStack); updateHtml(computerCards); }, 1200);
        ...
        if (computerCards[computerCards.length - 1].cardColor == currentCard.cardColor || computerCards[computerCards.length - 1].cardValency == currentCard.cardValency) {
            ...
            setTimeout(function(): void {document.getElementById(currentCard.cardColor + currentCard.cardValency).classList.add("cardtransition"); }, 1300);
            
            setTimeout(function(): void {computerCards.splice(computerCards.length - 1, 1); document.getElementById("currentMove").innerHTML = "Der Gegner legt eine Karte ab!"; }, 1300);
            setTimeout(function (): void {updateHtml(computerCards); updateHtml(activeCards); }, 2300);
        }
        // Dann ist der Spieler wieder an der Reihe
        setTimeout(function(): void {document.getElementById("currentMove").innerHTML = "Du bist dran!"; currentPlayer = true; }, 2300);
...
function createOpenCardHtml (array: Card[], arrayIndex: number, classString: string): void {
    ...
    // Wertigkeit ist zwei mal auf der Karte abgebildet
    let i: number  = 2;
    while (i) {
    ...
    if (classString == "player") {
        holdingDiv.addEventListener("click", function (): void {playThisCard(array[arrayIndex], arrayIndex); }, false);   
    }
...
function createHiddenCardHtml (array: Card[], arrayIndex: number, classString: string): void {
    ...
    image.setAttribute("src", "cards/Back/001.png");
...
function shuffle (array: Card[]): any {
    let m: number  = array.length, t: any, i: number;
